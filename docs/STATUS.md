...
- Tuck, Mochi, Biscuit, Tavi, Drift, and Olive now use a genuine volume-first runtime. Each starts with a head ball above a chest ball, three balls per arm, optional rounded hat balls, and the same explicit 11-joint graph. Radial cloth envelopes derive the continuous shell and face. A variable-width quadratic sweep through each shoulder-elbow-hand chain produces one rounded, continuous arm cover instead of three visible joined lobes; preset recipes contain no authored silhouette paths. **Cover/Balls** exposes both construction stages in the preview.
- **Make one** creates a new mouthless catchlight Palari from a numeric seed, varying head/chest size and distance, arm volumes and reach, eye size and position, four rounded hat constructions, reviewed palettes, and motion timing. The same seed regenerates the same balls, joints, and cover after reload. A transform-only two-mass spring makes the chest lead, the head lag and overshoot, and both squash on opposite axes to preserve apparent volume. The arms follow from their shoulder pivots; gaze and blink remain independent. **Alive/Still** controls the simulation, `prefers-reduced-motion` prevents idle and tap motion, and local PNG export always serializes the neutral cover at 1024 × 1024.
...
- The responsive browser app provides a 6 × 4 desktop picker that reflows to five or four columns, a large companion preview, soft/circle/square frames, Cover/Balls inspection, non-repeating surprise selection, URL-linked IDs and generated seeds, and local 1024 × 1024 PNG download. Playwright verified the 1536 × 1024, 390 × 844, and 320 × 568 layouts, all 24 accessible choices, six matching generated picker covers, continuous arm silhouettes, visible head/chest/arm/hat balls with exactly 11 joints and no cover geometry in Balls view, measured chest-led/head-lagged tap frames, complete settling, motion on/off, stable seed reload, reduced-motion behavior, covered PNG export while Balls is visible, and zero horizontal overflow.
...
- Compact phone layout uses the available height below the header: 40% for a centered portrait preview, 20% for three vertically aligned six-color rows (background above two shirt rows), and 40% for a full-width, six-column portrait grid that scrolls vertically; wider layouts retain the complete control set.
...
1. Review the three-character V2 neutral editing-master pilot and decide whether to apply the same treatment to the remaining 38 V2 figures.
2. Keep source and stored-mask checksums synchronized when artwork changes.
3. Regenerate and visually review framing whenever source pixels change.
