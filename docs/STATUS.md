...
- **The public editor is deployed:** GitHub Pages serves the browser-only app at `https://coystan.github.io/palari-art/`. `npm run build:pages` creates its verified 226.8 MiB artifact containing 314 avatar WebPs, 1,395 mask WebPs, and 40 handbook WebPs with no PNG masters or audit layers.
- **The character handbook is complete locally:** an 80-page inclusive art and production handbook is available as a separate `/handbook/` Vite entry with 20 reviewed teaching plates, 40 checksum-linked WebPs, and a 210 × 270 mm PDF with 3 mm bleed. Its next push will publish it through the existing Pages workflow.
...
- A responsive 80-page Palari Character Design Handbook with desktop spreads, mobile single-page reading, keyboard navigation, deep links, embedded local fonts, and a downloadable 5–6 MiB print-production PDF.
