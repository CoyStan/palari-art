...
- Tuck, Mochi, Biscuit, Tavi, Drift, and Olive now use a genuine skeleton-first runtime. Each starts with the same explicit 11-joint graph—root, chest, head, two shoulder-elbow-hand chains, and two eye anchors—plus character-specific proportions. Continuous shell, face, arms, and catchlit eyes are derived from that graph; preset recipes contain no authored silhouette paths. **Cover/Bones** exposes both construction stages in the preview.
- **Make one** creates a new mouthless catchlight Palari from a numeric seed, three bounded skeleton families, reviewed palettes, and deterministic proportions. The same seed regenerates the same bones and cover after reload. Idle bounce, head lag, arm follow-through, gaze, and tap-to-hop share the skeleton pivots. **Alive/Still** controls idle motion, `prefers-reduced-motion` prevents the tap response, and local PNG export always serializes the neutral cover at 1024 × 1024.
...
- The responsive browser app provides a 6 × 4 desktop picker that reflows to five or four columns, a large companion preview, soft/circle/square frames, Cover/Bones inspection, non-repeating surprise selection, URL-linked IDs and generated seeds, and local 1024 × 1024 PNG download. Playwright verified the 1536 × 1024, 390 × 844, and 320 × 568 layouts, all 24 accessible choices, six matching generated picker covers, exactly 11 visible joints with no cover geometry in Bones view, motion on/off, the 680ms tap response, stable seed reload, reduced-motion behavior, covered PNG export while Bones is visible, and zero horizontal overflow.
