...
The V3 browser app builds the six catchlight characters and every seeded character volume-first. `src/v3/skeleton.ts` starts with one head ball above one chest ball, three balls for each arm, and optional hat balls. An explicit 11-joint graph links the volumes: root, chest, head, two shoulder-elbow-hand chains, and two eye anchors. Presets store only bounded ball dimensions, distances, eye placement, hat style, palette, and motion traits. They contain no shell or face paths.

`src/v3/cover.ts` stretches smooth radial envelopes over the head and chest balls to derive the continuous shelter and face region. Each arm uses a sampled quadratic centerline through its shoulder, elbow, and hand balls, with interpolated elliptical width and round caps. This produces one continuous sleeve-and-mitten silhouette instead of exposing three joined lobes. The six matching picker tiles use those covers too, rather than their earlier finished-image thumbnails. The **Cover/Balls** control exposes both stages in the live preview, including the actual head, chest, shoulder, elbow, hand, and hat volumes. The cover adds no mouth, emblem, cast shadow, or new semantic color.

The **Make one** action uses a numeric seed to vary head/chest size and distance, arm ball sizes and reach, eye size and position, one of four rounded hat constructions, a reviewed color pair, gaze, and motion timing. The seed stays in the URL, so reloading the same URL recreates the same balls, joints, and cover without an image-generation request. Generated and bundled rigged characters export the neutral cover locally as a 1024px PNG, even when the inspection view is showing balls.

`src/v3/useSoftBodyMotion.ts` treats the chest and head as two damped masses connected by a spring. The chest follows a gentle idle anchor; tapping injects an upward impulse into the chest first, so the head visibly lags, receives the transferred force, overshoots, and settles. Horizontal and vertical scale move in opposite directions during acceleration to preserve apparent volume, like two soft water balloons under one cloth. The arms follow the chest mass from their shoulder pivots. This solver runs with `requestAnimationFrame` and writes transforms only; blink and gaze remain lightweight CSS animation.

The **Still** control clears every physics transform immediately. `prefers-reduced-motion` prevents both idle simulation and tap impulses and reacts if the system preference changes while the page is open.

The volume model and its cover share the same chest, head, and shoulder pivots, so motion is applied to the structure rather than inferred from finished art. The rig uses native SVG, CSS, and browser animation APIs plus its small deterministic spring integrator. V3 does not ship Rive, Spine, PixiJS, Lottie, a third-party physics engine, or runtime image generation.
