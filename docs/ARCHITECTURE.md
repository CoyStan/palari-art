...
V3 generation is skeleton-first. `src/v3/skeleton.ts` builds an explicit 11-joint graph from bounded proportions: root, chest, head, two shoulder-elbow-hand chains, and two eye anchors. `src/v3/cover.ts` derives the continuous shell, face region, round arm strokes, and catchlit eyes from that graph. `src/v3/procedural.ts` stores only recipes, palettes, and motion parameters for the six catchlight presets and creates the same recipe structure from a numeric seed; it contains no preset silhouette paths. `ProceduralPalari.tsx` can render either the originating bones or the generated cover and uses the same head and shoulder pivots for transform-only idle bounce, head lag, arm follow-through, blinking, and gaze. A 680ms Web Animations API response handles taps. The URL preserves generated seeds, `prefers-reduced-motion` suppresses movement, and `download.ts` always serializes the neutral cover for local 1024px PNG export. No animation library or image model runs in the browser.
