...
V3 generation is volume-first. `src/v3/skeleton.ts` builds explicit head and chest balls, three balls for each arm, optional rounded hat balls, and an 11-joint graph with root, chest, head, two shoulder-elbow-hand chains, and two eye anchors. `src/v3/cover.ts` calculates radial cloth envelopes around the head and chest balls, then sweeps a variable-width quadratic sleeve through each shoulder-elbow-hand chain with rounded caps. This derives a continuous shell, face region, two single-piece arm covers, and catchlit eyes without exposing joined volume lobes. `src/v3/procedural.ts` stores only recipes, palettes, and motion parameters for the six catchlight presets and creates the same recipe structure from a numeric seed; it contains no preset silhouette paths. `ProceduralPalari.tsx` can render either the originating balls and joints or the generated cover. `useSoftBodyMotion.ts` integrates a damped chest mass and a coupled head mass with `requestAnimationFrame`: the chest receives tap impulses first, the head lags and overshoots, inverse-axis scaling preserves apparent volume, and the arms follow from their shoulder pivots. It mutates transforms only and runs only for the active preview. Blink and gaze remain CSS animations. The URL preserves generated seeds, **Still** and `prefers-reduced-motion` clear movement, and `download.ts` always serializes the neutral cover for local 1024px PNG export. No animation library, third-party physics engine, or image model runs in the browser.
