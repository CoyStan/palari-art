...
Palari V3 is the icon-first direction at `/v3/`: an expanded family of 24 friendly avatars, including six retained V2 silhouettes and 18 native characters. Moss, Poppy, and Sunny anchor the mouthless, emblem-free direction. The newest six use deterministic flat-first geometry, a solid navy field, measured micro-gradients, and one small ivory catchlight per pupil. Their live versions start with adjustable head and chest balls, three balls per arm, optional hat balls, and an explicit 11-joint graph; smooth covers, faces, arms, and eyes are derived afterward. A Cover/Balls switch exposes both stages. Each visible arm is one continuous curved sleeve rather than three joined lobes. A two-mass spring makes the chest lead, the head lag and overshoot, and both conserve apparent volume like coupled water balloons; the arms follow from their shoulder pivots. **Make one** varies ball size and spacing, arms, eyes, hats, color, and motion from a shareable numeric seed without an image-generation request. Visitors can pause motion, switch among soft, circle, and square frames, and download a local 1024px PNG. The ceramic and 3D work remains separate.
