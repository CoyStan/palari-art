...
- `npm run build:pages` creates a verified 222.6 MiB artifact at the `/palari-art/` base path containing 314 avatar WebPs, 1,395 runtime-mask WebPs, 40 gallery WebPs, and 36 Palari V2 WebPs with no PNG, PDF, or audit files.
- A separate ceramic-character editor is published at `/v2/`. It uses 12 reviewed transparent masters and deterministic material/characteristic masks under `public/palari-v2/`, with 36 checksum-linked browser WebPs under `public/palari-v2-web/`. It does not replace or share masking logic with the V1 portrait editor.
...
npm run palari-v2:web:generate # Regenerate 12 V2 sources and 24 lossless mask WebPs
