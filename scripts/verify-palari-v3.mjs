...
if (collection.version !== 1 || collection.avatars.length !== 12) problems.push("V3 collection must contain exactly 12 founding avatars.");
if (manifest.schemaVersion !== 1 || manifest.recipeVersion !== 1 || manifest.avatars.length !== 12) problems.push("V3 WebP manifest is incomplete.");
...
if (new Set(collection.avatars.map((avatar) => avatar.name)).size !== 12) problems.push("V3 avatar names must be unique.");
...
if (newDirectories.length !== 6 || newDirectories[0] !== "palari-v3-007" || newDirectories[5] !== "palari-v3-012") {
  problems.push("V3 delivery directory must contain only the six new avatars.");
}
...
if (problems.length) {
  ...
  console.log("Verified 12 V3 founding avatars, six curated V2 sources, six new native masters, and all WebP delivery checksums and dimensions.");
}
